...
`tools/list` at `https://gateway.pipeworx.io/mojang/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/username_to_uuid \
  -H 'Content-Type: application/json' \
  -d '{"username":"Notch"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/username_to_uuid`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
